...
const _$1 = /* @__PURE__ */ getDefaultExportFromCjs(fp);
const PERMISSION_FIELDS = ["action", "subject", "properties", "conditions"];
const sanitizePermissionFields = _$1.pick(PERMISSION_FIELDS);
const getDefaultPermission = () => ({
  conditions: [],
  properties: {},
  subject: null
});
const create = _$1.pipe(_$1.pick(PERMISSION_FIELDS), _$1.merge(getDefaultPermission()));
const addCondition = _$1.curry((condition, permission) => {
  const { conditions } = permission;
  const newConditions = Array.isArray(conditions) ? _$1.uniq(conditions.concat(condition)) : [condition];
  return _$1.set("conditions", newConditions, permission);
});
const getProperty = _$1.curry(
  (property, permission) => _$1.get(`properties.${property}`, permission)
);
...
const createValidateContext = (permission) => ({
  get permission() {
    return fp.cloneDeep(permission);
  }
});
...
const createWillRegisterContext = ({ permission, options }) => ({
  ...
  condition: {
    and(rawConditionObject) {
      ...
      if (fp.isArray(permission.condition.$and)) {
    ...
    or(rawConditionObject) {
      ...
      if (fp.isArray(permission.condition.$and)) {
        const orClause = permission.condition.$and.find(fp.has("$or"));
        if (orClause) {
...
const operations = fp.pick(allowedOperations, sift__namespace);
...
const caslAbilityBuilder = () => {
  ...
  return {
    can(permission) {
      ...
      return can(
        caslAction,
        fp.isNil(subject) ? "all" : subject,
        fields,
        fp.isObject(condition) ? condition : void 0
      );
    },
...
const newEngine = (params) => {
  ...
  const evaluate = async (params2) => {
    ...
    const resolveConditions = _$1.map(providers.condition.get);
    const removeInvalidConditions = _$1.filter(
      (condition) => _$1.isFunction(condition.handler)
    );
      ...
      return Promise.all(
        conditions2.map(async (condition) => ({
          ...
          result: await condition.handler(
            _$1.merge(options, { permission: _$1.cloneDeep(permission) })
          )
        }))
      );
    };
    const removeInvalidResults = _$1.filter(
      ({ result }) => _$1.isBoolean(result) || _$1.isObject(result)
    );
    ...
    const resultPropEq = _$1.propEq("result");
    const pickResults = _$1.map(_$1.prop("result"));
    if (evaluatedConditions.every(resultPropEq(false))) {
      return;
    }
    if (_$1.isEmpty(evaluatedConditions) || evaluatedConditions.some(resultPropEq(true))) {
      return register({ action, subject, properties });
    }
    const results = pickResults(evaluatedConditions).filter(_$1.isObject);
    if (_$1.isEmpty(results)) {
      return register({ action, subject, properties });
    }
    return register({
      action,
